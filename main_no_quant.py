...
Version without quantization for better stability
"""
...
def load_model(model_name, device):
    """Load and prepare the model without quantization"""
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        device_map=None,  # Don't use device_map for single GPU
...
def setup_training_args(output_dir, num_epochs=2, learning_rate=1e-4):
    ...
    return TrainingArguments(
        ...
        per_device_train_batch_size=1,  # Reduced batch size for FP16
        ...
        fp16=True,
        eval_strategy="steps",
        eval_steps=100,  # Increased eval steps
        save_strategy="steps",
        save_steps=500,
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune model for VLSP Track 6 (No Quantization)")
